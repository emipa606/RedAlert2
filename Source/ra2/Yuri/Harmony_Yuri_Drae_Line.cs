    ...
    public static class Harmony_Yuri_Drae_Line
    {
        public static void Postfix()
        {
            foreach (var obj in Find.Selector.SelectedObjects)
            {
                ...
                {
                    var becontroler = thing as Pawn;
                    var controler =
                        ModBaseRa2.Instance._controlstorage.BeControlerAndControler.TryGetValue(becontroler);
                    if (controler.Map == becontroler.Map)
                    {
                        HighDrawLineBetween(thing.Position.ToVector3(), controler.Position.ToVector3(),
                            MaterialPool.MatFrom(GenDraw.LineTexPath, ShaderDatabase.Transparent,
                                Color.magenta));
                    }
                }
                else if (ModBaseRa2.Instance._controlstorage.ControlerAndBeControler.ContainsKey(thing as Pawn))
                {
                    var controler = thing as Pawn;
                    var becontroler =
